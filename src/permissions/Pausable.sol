 ...
 * @author Implementation taken from Eigen Labs, modified by Byzantine Finance
 * @notice Contracts that inherit from this contract may define their own `pause` and `unpause` (and/or related) functions.
 * These functions should be permissioned as "onlyPauser" which defers to a `PauserRegistry` for determining access control.
 * @dev Pausability is implemented using a uint256, which allows up to 256 different single bit-flags; each bit can potentially pause different functionality.
 * For the `pause` and `unpause` functions we've implemented, if you pause, you can only flip (any number of) switches to on/1 (aka "paused"), and if you unpause,
 * you can only flip (any number of) switches to off/0 (aka "unpaused").
...
contract Pausable is IPausable {
    ...
    /// @dev Paused status of the contract's functionality.
    ...
    /**
     * @notice This function is used to pause a Byzantine contract's functionality.
    ...
    /**
     * @notice This function is used to unpause a Byzantine contract's functionality.
